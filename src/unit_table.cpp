...
float unittable::unittable::get_unit_conversion(const size_t &from_hash, const size_t &to_hash) {
  ...
  m_stack.emplace(from_hash);
  float output(-1.);

  while (!m_stack.empty()) {
    curr_unit = m_stack.top();
    m_stack.pop();
    m_visited.emplace(std::pair<size_t, std::pair<float, size_t>>(curr_unit, std::pair<float, size_t>(this->m_table[prev_unit][curr_unit], prev_unit)));
    prev_unit = curr_unit;
